...
describe("Auth Controller - Login", () => {
  ...
  it("should send an OTP for the first login", async () => {
    const mockUser = {
      _id: "123",
      email: "test@example.com",
      password: "$2a$10$e2vYTrf1z3vAqGhvnklo1ubGJmFDuFclDCk3InFvXiQlrk3HZ0m1y", // mot de passe haché
      isVerified: true,
      otp: undefined, // Pas d'OTP existant
    ...
    jest.spyOn(jwt, "sign").mockReturnValue("mockToken");
    jest.spyOn(otpGenerator, "generateOTP").mockReturnValue("123456"); // Mock de la génération d'OTP
    ...
    expect(mockUser.save).toHaveBeenCalled(); // Vérifiez que l'utilisateur a été sauvegardé
  });
  ...
  it("should login a user and generate a JWT token for subsequent logins", async () => {
    const mockUser = {
      ...
      password: "$2a$10$e2vYTrf1z3vAqGhvnklo1ubGJmFDuFclDCk3InFvXiQlrk3HZ0m1y", // mot de passe haché
      isVerified: true,
      otp: "123456", // OTP valide
      otpExpires: Date.now() + 5 * 60 * 1000, // OTP valide
  ...
  it("should return an error if user is not verified", async () => {
    const mockUser = {
      _id: "123",
      email: "test@example.com",
      password: "$2a$10$e2vYTrf1z3vAqGhvnklo1ubGJmFDuFclDCk3InFvXiQlrk3HZ0m1y", // hashed password for 'Password123'
      isVerified: false,
      save: jest.fn().mockResolvedValue(),
    };
